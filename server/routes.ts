...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/notifications/preferences", async (req, res) => {
    try {
      ...
      const { push_enabled, booking_updates, payment_alerts, chat_messages } = req.body;

      // Validate boolean fields
      const updates: any = { updated_at: new Date().toISOString() };
      if (typeof push_enabled === 'boolean') updates.push_enabled = push_enabled;
      if (typeof booking_updates === 'boolean') updates.booking_updates = booking_updates;
      if (typeof payment_alerts === 'boolean') updates.payment_alerts = payment_alerts;
      if (typeof chat_messages === 'boolean') updates.chat_messages = chat_messages;
